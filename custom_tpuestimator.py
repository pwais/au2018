...
def my_model(features, labels, mode, params):
  ...
  for units in [50, 50, 50, 50, 50]:#params["hidden_units"]:
    net = tf.layers.dense(net, units=units, activation=tf.nn.relu)

  # Compute logits (1 per class).
  logits = tf.layers.dense(net, params["n_classes"], activation=None)
  tf.contrib.summary.histogram('logits', logits)
  tf.contrib.summary.scalar('logits_mean', tf.reduce_mean(logits))
...
def main(argv):
  ...
  tpu_cluster_resolver = tf.contrib.cluster_resolver.TPUClusterResolver(
      FLAGS.tpu)
  train_distribution = tf.contrib.tpu.TPUDistributionStrategy(
      tpu_cluster_resolver, steps_per_run=FLAGS.batch_size * 8)
