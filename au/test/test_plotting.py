...
def test_hash_to_rgb():
  assert aupl.hash_to_rbg('moof') == (1, 2, 3)
  assert aupl.hash_to_rbg(5) == (1, 2, 3)
...
def test_spark_histogram():
  with testutils.LocalSpark.sess() as spark:
    ...
    hist, edges = aupl.histogram(df, 'a', 1)
    check(
      hist,   np.array([101]),
      edges,  np.array([0., 100.]))

    hist, edges = aupl.histogram(df, 'a', 2)
    check(
      hist,   np.array([50, 51]),
      edges,  np.array([0., 50., 100.]))

    hist, edges = aupl.histogram(df, 'b', 4)
    check(
      hist,   np.array([50, 51, 1, 1]),
      edges,  np.array([0., 50., 100., 1, 1]))
...
def test_histogram_with_examples_basic():
  with testutils.LocalSpark.sess() as spark:
    ...
    pl = HistogramWithExamplesPlotter()
    fig = pl.run(spark, df, 'a')
    save_bokeh_fig(fig, '/opt/au/tasttast.html')
